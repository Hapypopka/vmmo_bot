...
def check_and_use_stalker_seal(page):
    """
    Проверяет, есть ли подсказка "Печать Сталкера" и активирует её если не заморожена.
    Вызывать в основном цикле боя когда нет врагов.
    Возвращает True если печать была активирована.
    """
    try:
        # Проверяем есть ли подсказка про Печать Сталкера
        page_text = page.inner_text("body")
        if "применить Печать Сталкера" not in page_text:
            return False

        log("🎃 Обнаружена подсказка про Печать Сталкера!")

        # Ищем кнопку Печати Сталкера (device-pos-1)
        seal_wrapper = page.query_selector(".wrap-device-link._device-pos-1")
        if not seal_wrapper:
            log("⚠️ Кнопка Печати не найдена")
            return False

        # Проверяем, не заморожена ли (класс _freeze)
        wrapper_class = seal_wrapper.get_attribute("class") or ""
        if "_freeze" in wrapper_class:
            log("❄️ Печать заморожена — ждём...")
            return False

        # Печать готова — кликаем!
        seal_link = seal_wrapper.query_selector("a.device-link")
        if seal_link:
            seal_link.dispatch_event("click")
            log("🎃 Активировали Печать Сталкера!")
            antibot_delay(2.0, 0.5)
            return True

        return False

    except Exception as e:
        log(f"⚠️ Ошибка активации Печати: {e}")
        return False
