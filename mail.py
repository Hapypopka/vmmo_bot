...
def process_mailbox(page):
    ...
    while processed_count < max_messages:
        ...
        if processed_count > 0:
            # Возвращаемся в профиль, потом в почту
            if not open_profile(page):
                break
            if not open_mailbox(page):
                break
        ...
        if result == "backpack_full":
            ...
            # Возвращаемся в профиль -> почту -> сообщение
            if not open_profile(page):
                break
            if not open_mailbox(page):
                break
...
def check_and_collect_mail(page):
    ...
    # Открываем профиль
    if not open_profile(page):
        log("⚠️ Не удалось открыть профиль")
        return False

    # Открываем почтовый ящик
    if not open_mailbox(page):
        log("⚠️ Не удалось открыть почту")
        return False
