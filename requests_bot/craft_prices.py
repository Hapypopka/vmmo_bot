...
# проверен. Раньше флудили и убрали. Вернули как пробу с 1 бота.
# 2026-07-03: проба зелёная (sell-through 100%, 0 протуха, медиана ~2.5ч) →
# осторожно масштабируем до 2 ботов, продолжаем мерить.
# 2026-07-06: на 2 ботах рынок затопило — sell-through рухнул 100%→67%→29%→
# 14%→0% за 4 дня, 45 протуха. Спрос не тянет даже 2. Откат до 1.
RECIPE_BOT_CAP = {
    "copperBar": 1,
}
