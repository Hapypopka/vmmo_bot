...
# 2026-06-03: thor и thorBar убраны — не продаются (0/800 за 14д). Юзер
# не хочет их крафтить. Уже накопленные Тор продолжают выставляться (auction.py
# трогать не надо, лот может когда-нибудь выкупиться).
# 2026-06-04: bronzeBar убран — невыгоден по решению юзера.
FINAL_RECIPES = [
...
# 2026-07-13: bronze выведена из исключений как PROBE (cap=1): теор профит
# 1.28з/ч — выше всех (железо 0.93). Спрос неизвестен — меряем неделю.
# 2026-07-20: copper/copperBar — в исключения. За неделю 14-19.07:
#   медный слиток sell-through 22% (101 лот протух), медь 44% — рынок мёртв,
#   два бота работали в мусорку. Боты уйдут сами на границе партии.
AUTO_SELECT_EXCLUDED = {"platinumBar", "copper", "copperBar"}

# Демо-тюненная таблица капов на рецепт (переопределяет time-based
# get_max_bots_for_recipe). Используется двояко:
#   1) PROBE — рецепт с НЕИЗВЕСТНЫМ спросом крафтит ровно N ботов, меряем
#      sell-through, потом масштабируем/выкидываем.
#   2) BALANCE — подрезаем перепроизводство и поднимаем потолок там, где
#      спрос не выбран (sell-through высокий на текущем числе ботов).
# Сумма капов держится = числу крафтеров (23), иначе аллокатор в финальном
# equal-distribution проходе разольёт лишних БЕЗ учёта капов.
#
# 2026-07: copperBar — топ по теории, но проба на 2 ботах затопила рынок
#   (sell-through 100%→0% за 4д, 45 протуха) → откат до 1.
# 2026-07-06: перебалансировка по реальной з/час (теор × sell-through 7д):
#   медь 0.01, медн.слиток 0.11 — почти вхолостую; железо 0.73, руда 0.42 —
#   упёрлись в потолок cap=5 при спросе 78-84%. Льём мёртвый труд в железную
#   ветку: copper 3→1, платина сама до капа 3, iron 5→6, rawOre 5→7.
# 2026-07-13: bronze — PROBE cap=1 (теор 1.28з/ч, топ по теории; спрос
#   неизвестен — как copperBar в июле). Через неделю: sell-through > 80%
...
RECIPE_BOT_CAP = {
    ...
    "bronze": 2,      # спрос 82% — масштабируем пробу
}
