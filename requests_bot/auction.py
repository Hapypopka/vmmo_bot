...
DEFAULT_PRICES = {
    ...
    # 2026-06-03: Тор удалён — он реально не продаётся (0 продаж за 14 дней,
    # 800+ истекших лотов). Через CRAFT_ITEM_NAMES он был exempt от blacklist,
    # теперь убран оттуда тоже → попадает в auction_blacklist при первом fail.
}
...
CRAFT_ITEM_NAMES = {
    ...
    "platinum": "Платина",
    # 2026-06-03: Тор и Слиток Тора убраны — не продаются (0/800 за 14д).
    # Слиток Тора защищён через PROTECTED_ITEMS (там есть).
}
