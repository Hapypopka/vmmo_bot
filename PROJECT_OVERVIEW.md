...
|---|----------|------|----------|
| 1 | **Избыточные проверки крафта** | bot.py | `check_craft()` вызывается 11+ раз за цикл. Каждый вызов: загрузка конфига + проверка времени + возможный HTTP запрос |
| 2 | **Глобальное состояние конфига** | config.py | `_profile_config` - мутируемый глобальный dict. Race condition при параллельном доступе |
| 3 | **Хрупкие URL через regex** | combat.py, run_dungeon.py | URL'ы строятся через regex-извлечение page_id. Любое изменение API сломает всё |

### 10.2 СРЕДНИЕ

| # | Проблема | Файл | Описание |
|---|----------|------|----------|
| 4 | **Фрагментация сбора лута** | combat.py | 3 разных метода сбора с fallback-цепочкой. Непонятно какой работает |
| 5 | **Жизненный цикл CraftClient** | bot.py | Создаётся один раз при старте. Если рецепт изменится - не обновится |
| 6 | **Смешанная конфигурация** | config.py | Часть настроек в профиле, часть глобальная. Непоследовательное сохранение |
| 7 | **Неполное восстановление** | bot.py | Watchdog сбрасывает, но не восстанавливает состояние. Hell Games не триггерит воскрешение |

### 10.3 НЕЗНАЧИТЕЛЬНЫЕ

| # | Проблема | Файл | Описание |
|---|----------|------|----------|
| 8 | **Мёртвый код** | backpack.py | `repeat_craft_if_ready()` - deprecated, не используется |
| 9 | **Мёртвый код** | bot.py | `try_event_dungeon()` - отключён, не вызывается |
| 10 | **Мёртвый код** | craft.py | `craft_ore_until_ready()`, `craft_iron_until_ready()` - не вызываются |
| 11 | **Magic numbers** | везде | `attack_count % 3`, `BACKPACK_THRESHOLD = 18`, `WATCHDOG_TIMEOUT = 90` |
| 12 | **Импорты** | bot.py | 30+ прямых импортов, сложно поддерживать |
...
| Приоритет | Что | Как |
|-----------|-----|-----|
| HIGH | Кэшировать craft_finish_time | Хранить в памяти, обновлять только при изменении |
| HIGH | Убрать глобальное состояние | Сделать Config классом с методами |
| HIGH | Централизовать URL-билдинг | Вынести в отдельный модуль с константами |
| MEDIUM | Унифицировать сбор лута | Оставить только refresher, удалить fallback'и |
| MEDIUM | Документировать автосохранение | Указать какие файлы сохраняются автоматически |
| LOW | Удалить мёртвый код | repeat_craft_if_ready, try_event_dungeon, etc. |
| LOW | Добавить type hints | Хотя бы для публичных методов |
