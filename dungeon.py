...
def clear_blocking_widget(page):
    ...
    try:
        ...
        if enter_btn:
            ...
            # Вход не удался (нет "Начать бой" или инстанс багнутый) — покидаем банду
            log("⚠️ Вход закрыт — покидаем банду...")

            # СНАЧАЛА выходим в /dungeons — там появится виджет с "Покинуть банду"
            log("🏰 Переходим в подземелья для выхода из банды...")
            page.goto(DUNGEONS_URL)
            time.sleep(4)
            antibot_delay(1.0, 1.0)

            # Теперь ищем кнопку "Покинуть банду" в виджете на странице подземелий
            leave_buttons = page.query_selector_all("a.go-btn")
            for btn in leave_buttons:
                if "Покинуть банду" in btn.inner_text():
                    btn.dispatch_event("click")
            ...
            # Если кнопки нет — просто сбрасываем счётчик
            log("⚠️ Кнопка 'Покинуть банду' не найдена")
            _widget_enter_attempts = 0
            return True
